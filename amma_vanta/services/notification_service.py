...
<div class="w">
  ...
  <div class="b">
    ...
    <div class="ob">
      ...
      {% for item in items %}
      <div class="ir">
        <span>{{ item.emoji }} {{ item.name }}{% if item.weight %} ({{ item.weight }}){% endif %} × {{ item.qty }}</span>
  ...
  <div class="b">
    ...
    <table>
      ...
      <tbody>
        {% for item in items %}
        <tr>
          <td>{{ item.emoji }} {{ item.name }}{% if item.weight %} ({{ item.weight }}){% endif %}</td>
          <td>× {{ item.qty }}</td>
          <td>₹{{ item.total }}</td>
        </tr>
        {% endfor %}
...
def _mail_configured():
    """Check if MAIL_USERNAME and MAIL_PASSWORD are properly set in config."""
    try:
        username = current_app.config.get('MAIL_USERNAME', '').strip()
        password = current_app.config.get('MAIL_PASSWORD', '').strip()
        placeholders = ('', 'your-gmail-app-password-here', 'your_password', 'change_me')
        if not username:
            print("[EMAIL] ⚠️  MAIL_USERNAME not set in .env — emails skipped.")
            return False
        if not password or password.lower() in placeholders:
            print("[EMAIL] ⚠️  MAIL_PASSWORD not set in .env — emails skipped.")
            print("[EMAIL]    → Gmail App Password generate చేసి .env లో MAIL_PASSWORD= లో paste చేయండి.")
            return False
        return True
    except Exception:
        return False
